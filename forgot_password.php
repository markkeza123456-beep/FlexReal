  ...
  <main class="container">
    <div class="card">
      ...
      <div id="step2" class="step-2">
        ...
        <p class="subtitle">กรอกรหัส PIN และรหัสผ่านใหม่</p>
  ...
  <script>
    ...
    document.getElementById('forgotForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const text = await res.text(); // อ่านข้อความดิบก่อน
            try {
                ...
                // ถ้าไม่ใช่ JSON แสดงว่า PHP พัง! ให้ Alert บอกเลย
                alert("PHP Error ค้นพบปัญหา:\n" + text);
            }
        } catch(e) {
            showToast('การเชื่อมต่อล้มเหลว', 'error-toast');
        } finally {
            btn.disabled = false; btn.innerText = 'ขอรหัส PIN';
        }
    });
    ...
    document.getElementById('resetForm').addEventListener('submit', async (e) => {
        ...
        const res = await fetch('reset_password_action.php', {
            method: 'POST',
            body: new URLSearchParams({ 'pin': pin, 'new_password': pass })
        });
