(() => {
  ...
  function render() {
    ...
    $('videoTableBody').innerHTML = videos.length ? videos.map((video) => `<tr><td><a class="resource-link video" href="${esc(video.url)}" target="_blank" rel="noopener noreferrer">${esc(video.title)}</a></td><td>${esc(video.lesson_title || 'วิดีโอรวมของรายวิชา')}</td><td>${duration(video.duration_seconds)}</td><td>${esc(video.display_order)}</td><td><div class="action-btns"><button class="btn-icon" type="button" data-edit="${esc(video.id)}">แก้ไข</button><button class="btn-icon danger" type="button" data-delete="${esc(video.id)}">ลบ</button></div></td></tr>`).join('') : '<tr><td colspan="5" class="empty-cell">ยังไม่มีวิดีโอในรายวิชานี้</td></tr>';
  }
  async function load() {
