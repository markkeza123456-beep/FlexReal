...
function ensureLearningProgressTables(PDO $conn): void {
    // Schema changes must be applied by a deployment migration, never on a live
    // request.  ALTER TABLE obtains locks and was causing all learners to queue
    // behind each other under concurrent traffic.  See docs/database-performance.sql.
}
...
function recordLearningActivity(PDO $conn, string $studentId, string $courseId, int $lessonIndex, string $activityType, string $lessonTitle = '', float $progressPercent = 0, float $resumePosition = 0): void {
    ...
    // PDO serializes false as an empty string. PostgreSQL boolean columns reject that,
    // so use their explicit text literals for the CASE conditions below.
